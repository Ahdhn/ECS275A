...
Material *Parser::parsePhongMaterial()
{
	...
	return new PhongMaterial(color, is_luminous, is_reflective, Ka, Kd, exponent, Kr);
}
...
Object *Parser::parseSphereObject()
{
	...
	Sphere* sp = new Sphere(material, is_luminous, sampling_freq, center, radius, direction, speed);
	if (is_luminous){
		myAreaLights.push_back(sp);
	}
	return sp;
}
...
Object *Parser::parsePolygonObject()
{
	...
	//Primitive* poly = new Polygon(material, point_list);
	Primitive* poly = new Polygon(material, is_luminous, sampling_freq, point_list, direction, speed);
	if (is_luminous){
		myAreaLights.push_back(poly);
	}
	return poly;
}
...
Scene *Parser::parseScene(string &filename)
{
	...
	//add the area light after setting the image 
	for (int i = 0; i < myAreaLights.size(); i++){
		scene->addAreaLight(myAreaLights[i]);
	}
