...
void Scene::render()
{
	...
	std::vector< std::vector<Color>>myBuffer2;
	myBuffer2.resize(yres);
	for (int i = 0; i < yres; i++){
		myBuffer2[i].resize(xres);
	}

	//path tracing 
	//initalize accumlator buffer with zeros 
	std::vector< std::vector<Color>>myBuffer;
	myBuffer.resize(yres);
	for (int i = 0; i < yres; i++){	
		myBuffer[i].resize(xres);
	}
	for (int i = 0; i < yres; i++){
		for (int j = 0; j < xres; j++){			
			myBuffer[i][j] = Color(0, 0, 0);
		}
	}
	for (int ittr = 1; ittr <=path_trace_freq; ittr++){//do as many iteration as path_trace_freq
		printf("\n iteration=%d", ittr);

		for (int j = 0; j < yres; j++){
			for (int i = 0; i < xres; i++){
				printf("\rRendering pixel (%d,%d)", i, j);
				fflush(stdout);
				...
				//	int ss = 454;
				//}
				std::vector<Point2D>pl = PixelSampling(i, j, xres, yres, context);
				std::vector<Ray>rays;				
				//camera->makeRay(rays, context, pl);
				camera->makeRay(rays, context, pl[0].x, pl[0].y); 
				...
				for (int k = 0; k < pl.size(); k++){
					...
					if (hit.getPrimitive()){
						...
						matl->shade(mycolor, context, rays[k], hit, atten, 0);
					}
					else{
						background->getBackgroundColor(mycolor, context, rays[k]);
					}
					myBuffer[i][j] = myBuffer[i][j] + mycolor; 
				}
			}
		}
		...
		if (ittr % 2 == 0 || ittr==1) {
			for (int i = 0; i < yres; i++){
				for (int j = 0; j < xres; j++){
					myBuffer2[i][j] = myBuffer[i][j] / (double)ittr;
					myBuffer2[i][j].cut();
					image->set(i, j, myBuffer2[i][j]);
				}
			}										
			stringstream ss;
			ss << "image_" << ittr << ".ppm";
	...
	for (int j = 0; j < yres; j++){
		for (int i = 0; i < xres; i++){
			myBuffer[i][j] = myBuffer[i][j] / double(path_trace_freq);
			myBuffer[i][j].cut();
			image->set(i, j, myBuffer[i][j]);
		}
	}
	

	//ray tracing with anti-aliasing, motion blur, and depth of field 
