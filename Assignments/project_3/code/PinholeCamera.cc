...
void PinholeCamera::makeRay(std::vector<Ray>& rays, const RenderContext& context, const std::vector<Point2D> subpixels) const
{
	/*Ray myRay;


	makeRay(myRay, context, x, y);
	rays.clear();
	rays.push_back(myRay);*/


}
void PinholeCamera::makeRay(std::vector<Ray>& rays, const RenderContext& context, double x, double y) const
{
	Ray myRay;
	makeRay(myRay, context, x, y);
	rays.clear();
